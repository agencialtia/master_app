...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-white dark:bg-slate-900 border-b border-slate-200/90 dark:border-slate-800 shadow-[0_1px_3px_rgba(0,0,0,0.02)] transition-colors duration-200">
      <div className="max-w-[1780px] mx-auto px-3 sm:px-6 lg:px-10 xl:px-12 h-14 sm:h-18 flex items-center justify-between gap-2 sm:gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0 select-none">
          ...
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-br from-blue-600 via-indigo-600 to-violet-600 p-1.5 sm:p-2.5 shadow-md shadow-blue-500/15 flex items-center justify-center text-white shrink-0">
          ...
          {/* Logo Name & Subtitle */}
          <div>
            <div className="flex items-center text-sm sm:text-lg tracking-tight leading-none">
              <span className="font-extrabold text-slate-950 dark:text-white whitespace-nowrap">Master App</span>
            </div>
            <span className="text-[10px] sm:text-[11px] font-semibold text-slate-500 dark:text-slate-400 block leading-tight mt-0.5 hidden md:block">
        ...
        <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
          ...
          {/* 1. Language Indicator: Hidden on small mobile to give room for key actions */}
          <div className="hidden sm:flex items-center gap-1 text-slate-500 dark:text-slate-400 text-xs font-semibold px-2 py-1 select-none">
            ...
            <Globe size={14} className="text-slate-500 dark:text-slate-400 stroke-[1.8]" />
          </div>
          ...
          <div className="relative" ref={userMenuRef}>
            <button
              ...
              className="flex items-center gap-1 p-1 rounded-full hover:bg-slate-100/80 dark:hover:bg-slate-800 transition-all cursor-pointer focus:outline-none"
              title="Cuenta de usuario"
              aria-expanded={isUserMenuOpen}
            >
              <div className="w-7 h-7 sm:w-9 sm:h-9 rounded-full bg-[#6347EA] hover:bg-[#5237DB] flex items-center justify-center text-white text-xs sm:text-sm font-black shadow-xs transition-transform active:scale-95">
                {avatarInitial}
              </div>
              <ChevronDown size={13} className="text-slate-500 dark:text-slate-400 stroke-[2.2] transition-transform duration-150" />
            </button>

            {/* Dropdown Menu */}
            {isUserMenuOpen && (
              <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200/90 dark:border-slate-800 py-1.5 z-50 animate-in fade-in zoom-in-95 duration-100">
                {/* User Info Header */}
