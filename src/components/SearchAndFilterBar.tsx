...
export const SearchAndFilterBar: React.FC<SearchAndFilterBarProps> = ({
  ...
  const hasActiveFilters = Boolean(searchQuery || selectedProjectId || selectedStatus || showOnlyFavorites);
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/90 dark:border-slate-800 shadow-xs p-3 sm:p-4 space-y-2.5 mb-5 transition-colors duration-200">
      ...
      {/* 2. Side-by-Side Compact Filters: PROYECTOS, ESTADO & FAVORITOS */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3 items-end">
        
        {/* A. Botón con Menú Desplegable "PROYECTOS" */}
        <div className="relative" ref={dropdownRef}>
          <label className="block text-[10px] font-bold uppercase tracking-wider mb-1 flex items-center gap-1 text-blue-700 dark:text-blue-400">
          ...
          <button
            ...
            className={`w-full h-9 sm:h-10 flex items-center justify-between px-2.5 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-750 border rounded-lg sm:rounded-xl text-xs font-semibold transition-all cursor-pointer text-left ${
              selectedProjectId 
        ...
        {/* B. Filtro: ESTADO */}
        <div>
          <label className="block text-[10px] font-bold uppercase tracking-wider mb-1 flex items-center gap-1 text-emerald-700 dark:text-emerald-400">
          ...
          <div className="relative">
            <select
              ...
              className={`w-full h-9 sm:h-10 px-2.5 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-750 border rounded-lg sm:rounded-xl text-xs font-semibold transition-all cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500/20 appearance-none pr-7 truncate ${
                selectedStatus 
        ...
        {/* C. Botón: FAVORITOS */}
        <div>
          <label className="block text-[10px] font-bold uppercase tracking-wider mb-1 flex items-center gap-1 text-amber-600 dark:text-amber-400">
          ...
          <button
            ...
            className={`w-full h-9 sm:h-10 flex items-center justify-between px-2.5 rounded-lg sm:rounded-xl text-xs font-bold transition-all cursor-pointer border ${
              showOnlyFavorites
                ? 'bg-amber-50 dark:bg-amber-950/60 border-amber-400 dark:border-amber-500 text-amber-900 dark:text-amber-200 ring-2 ring-amber-400/30 shadow-xs'
                : 'bg-slate-50 dark:bg-slate-800 hover:bg-amber-50/70 dark:hover:bg-amber-950/30 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:border-amber-300 dark:hover:border-amber-700'
            }`}
            title={showOnlyFavorites ? 'Mostrar todos los proyectos' : 'Ver solo proyectos favoritos'}
          >
            <div className="flex items-center gap-2">
              <div className={`w-5 h-5 rounded-md flex items-center justify-center transition-transform ${
                showOnlyFavorites ? 'bg-amber-400 text-white scale-105' : 'bg-amber-100/80 dark:bg-amber-900/60 text-amber-600 dark:text-amber-400'
              }`}>
                <Star size={12} className="fill-amber-400 text-amber-500" />
              </div>
              <span>Favoritos</span>
      ...
      {/* 3. Botón "Nuevo" debajo de Proyectos, Estado y Favoritos */}
      {onNewProject && (
        <button
      ...
      {hasActiveFilters && (
        <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2 text-xs">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {showOnlyFavorites && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 text-[10px] font-bold border border-amber-300/60 dark:border-amber-700/60">
