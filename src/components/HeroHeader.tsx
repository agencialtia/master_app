...
interface HeroHeaderProps {
  ...
  onQuickFilter: (type: 'all' | 'active' | 'favorites') => void;
  viewMode: string;
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 text-xs tracking-widest font-semibold uppercase text-slate-500 dark:text-slate-400 mb-2">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {favoriteCount > 0 && (
              <button
                type="button"
                onClick={() => onQuickFilter(showOnlyFavorites ? 'all' : 'favorites')}
                className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-bold tracking-wider transition-all cursor-pointer border ${
                  showOnlyFavorites
                    ? 'bg-amber-100 dark:bg-amber-950/80 text-amber-900 dark:text-amber-200 border-amber-400 dark:border-amber-600 ring-2 ring-amber-400/30'
                    : 'bg-amber-50/80 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 border-amber-200/70 dark:border-amber-800/60 hover:bg-amber-100 dark:hover:bg-amber-950/70'
                }`}
                title={showOnlyFavorites ? 'Ver todos los proyectos' : 'Filtrar proyectos favoritos'}
              >
                <Star size={12} className="fill-amber-400 text-amber-500" />
                <span>{favoriteCount} {favoriteCount === 1 ? 'FAVORITO' : 'FAVORITOS'}</span>
              </button>
            )}
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 border border-emerald-200/60 dark:border-emerald-800/60 text-[11px] font-bold tracking-wider hover:bg-emerald-100/70 dark:hover:bg-emerald-950/60 transition-colors cursor-pointer"
              title="Filtrar proyectos activos"
            >
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
              <span>{activeCount} {activeCount === 1 ? 'PROYECTO ACTIVO' : 'PROYECTOS ACTIVOS'}</span>
            </button>
          </div>
        </div>
