...
import { getCurrentAuthUser, setCurrentAuthUser } from './utils/authStorage';
...
export default function App() {
  ...
  // Search & Filter State - Proyectos, Estado, and Favoritos
  ...
  const favoritesCount = useMemo(() => {
    return projects.filter(p => p.favorite).length;
  }, [projects]);

  // Filtered Projects (Filtered by Selected Project, Selected Status, Search Query, and Favoritos)
  const filteredProjects = useMemo(() => {
    return projects.filter(project => {
      // 0. Favorites filter
      if (showOnlyFavorites && !project.favorite) {
        return false;
      }
  ...
  }, [projects, showOnlyFavorites, selectedProjectId, selectedStatus, searchQuery]);
  ...
  const handleQuickFilter = (type: 'all' | 'active' | 'favorites') => {
    handleResetFilters();
    if (type === 'active') {
  ...
  const handleLoginSuccess = (user: AuthUser) => {
    ...
    // Sync profile and username
    setUserName(user.name);
    setUserProfile(prev => ({
      ...prev,
      name: user.name,
      email: user.email,
      avatarText: user.avatarText || user.name[0].toUpperCase()
    }));
  };
    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col selection:bg-blue-500 selection:text-white transition-colors duration-200">
      ...
      <main className="flex-1 max-w-[1780px] w-full mx-auto px-4 sm:px-6 lg:px-10 xl:px-12 py-6">
        ...
        {filteredProjects.length === 0 ? (
          /* Empty State */
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-8 sm:p-12 text-center max-w-lg mx-auto shadow-xs animate-in fade-in duration-150">
            {showOnlyFavorites ? (
              <>
                <div className="w-16 h-16 rounded-2xl bg-amber-50 dark:bg-amber-950/60 border border-amber-200 dark:border-amber-800/80 text-amber-500 mx-auto flex items-center justify-center mb-4 shadow-xs">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mb-6 leading-relaxed">
                  Haz clic en el ícono de estrella (★) en cualquier tarjeta de proyecto para agregarlo automáticamente a tus favoritos.
                </p>
                <div className="flex items-center justify-center gap-3">
                  <button
                    onClick={() => setShowOnlyFavorites(false)}
                    className="inline-flex items-center gap-1.5 px-4 py-2.5 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-xs font-bold shadow-sm transition-all cursor-pointer"
                  >
